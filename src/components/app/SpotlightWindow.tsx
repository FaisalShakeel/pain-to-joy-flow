  ...
  return (
    <section className="rounded-[1.35rem] bg-gradient-to-br from-primary/[0.04] via-surface-lowest to-surface-low/40 ghost-border shadow-soft overflow-hidden">
      ...
      <div className="px-4 md:px-5 pt-3">
        <div className={cn(
          "rounded-2xl ghost-border bg-surface-lowest transition-all ease-premium",
          expanded ? "p-3 shadow-soft" : "p-2",
        )}>
          {!expanded ? (
            <button
              type="button"
              onClick={() => setExpanded(true)}
              className="w-full flex items-center gap-3 text-left px-2 py-1.5 rounded-xl hover:bg-surface-low/60 transition"
            >
              <span className="grid place-items-center w-8 h-8 rounded-full bg-primary text-primary-foreground shrink-0">
                <Send className="w-3.5 h-3.5" />
              </span>
              <span className="flex-1 text-xs md:text-sm text-muted-foreground truncate">
                Publish a spotlight, status, or quick availability note…
              </span>
              <span className="hidden sm:inline-flex items-center gap-1 text-[10px] font-semibold text-primary/70 uppercase tracking-wider">
